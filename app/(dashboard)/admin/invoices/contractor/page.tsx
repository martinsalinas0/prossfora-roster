...
const renderRow = (item: any) => (
  ...
  >
    ...
    <td className="hidden lg:table-cell">{item.contractor}</td>
...
const CustomerInvoicesForAdminPage = () => {
  ...
  const filteredInvoices = contractorInvoicesData.filter((invoice) => {
    const q = searchQuery.toLowerCase();
    return (
      invoice.invoiceNumber.includes(q) ||
      invoice.contractor.toLowerCase().includes(q)
    );
  });
